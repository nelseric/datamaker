...
class SimplePosition(Feature):
  ...
  def __init__(self, limit_upper = 0.00055, limit_lower = 0.00015, search_limit = 1440, *args, **kwargs):
    super(SimplePosition, self).__init__(*args, **kwargs)
  ...
  def _calculate(self, data):
    ...
    result.columns = ["SimplePosition"]
    return result


cpdef npc.ndarray apply(npc.ndarray[double, ndim=2] data, double margin_upper, double margin_lower, int limit):
  # """
  #   This does the actual computation for SimplePosition. It expects the data at the following indexes:
  #     0 Ask         open            1.084150
  #     1             high            1.084150
  #     2             low             1.083600
  #     3             close           1.083900
